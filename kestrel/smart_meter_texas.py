...
from kestrel.models import EnergyInterval, hash_identifier, interval_end_from_start, utc_now_iso
...
def parse_csv_content(
    ...
    *,
    ...
    for line_no, row in enumerate(reader, start=2):
        try:
            interval = _parse_csv_row(row, mapping, tz)
...
def _parse_csv_row(
    ...
    start_utc = start_local.astimezone(timezone.utc).replace(microsecond=0)
    end_utc = end_local.astimezone(timezone.utc).replace(microsecond=0)
    return _ParsedCsvRow(start_ts=start_utc.isoformat(), end_ts=end_utc.isoformat(), kwh=kwh)
