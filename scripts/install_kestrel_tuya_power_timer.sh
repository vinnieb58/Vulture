#!/usr/bin/env bash
# Install Kestrel Tuya power poll systemd timer on Raven.
# Does not print or read .env contents.
set -euo pipefail

APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
UNIT_DIR="${APP_DIR}/deploy/systemd"
SERVICE="kestrel-tuya-power-poll.service"
TIMER="kestrel-tuya-power-poll.timer"

usage() {
    cat <<'EOF'
Usage: ./scripts/install_kestrel_tuya_power_timer.sh [--enable]

Copy Kestrel Tuya power poll systemd units to /etc/systemd/system/, run daemon-reload,
and optionally enable/start the 60-second timer.

Options:
  --enable   Enable and start kestrel-tuya-power-poll.timer after install
  --help     Show this help
...
if [[ "$ENABLE" -eq 1 ]]; then
    ...
    systemctl list-timers --all | grep kestrel-tuya-power || true
else
