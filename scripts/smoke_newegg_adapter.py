#!/usr/bin/env python3
"""Smoke test for the Newegg adapter (experimental, no location targeting)."""
...
def _check_listing(listing, query: str, idx: int) -> str | None:
    ...
    if not listing.link.startswith("https://www.newegg.com/"):
...
def main() -> int:
    ...
    print(
        f"[INFO] newegg: experimental={caps.get('experimental')}, "
        f"stable={caps.get('stable')}, location_control={caps.get('location_control')}"
    )

    if get_adapter("newegg") is None:
        print("[FAIL] registry lookup returned None for newegg")
        return 1
    ...
    for query in _QUERIES:
        print(f"\n--- query: {query!r} ---")
        try:
            results = search_newegg(query, city=None, limit=10)
    ...
    if any_results:
        print("\n[PASS] Newegg smoke checks passed (at least one query returned listings)")
    else:
        print(
